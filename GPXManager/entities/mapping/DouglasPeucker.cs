    ...
    public static class DouglasPeucker
    {
        public static double LengthShapeSimplified()
        {
            ...
            for(int x=0;x<ShapeSimplified.numPoints;x++)
            {
                if(x>0)
                {
                    Waypoint wpt2 = new Waypoint { Longitude = ShapeSimplified.Point[x].x, Latitude = ShapeSimplified.Point[x].y, Elevation = 0 };
                    double elevChange = 0;
                   totalDistance +=  Waypoint.ComputeDistance(wpt1, wpt2, out elevChange);
                }
                wpt1 = new Waypoint { Longitude = ShapeSimplified.Point[x].x, Latitude = ShapeSimplified.Point[x].y, Elevation = 0 };

            }
            return totalDistance;
        }
        public static Shape ShapeSimplified { get; set; }
        /// <summary>
        ...
        public static Shape DouglasPeuckerReduction(Shape inShape, double ToleranceMeters)
        {
            ...
            {
                points.Add(new Point { X = inShape.Point[x].x, Y = inShape.Point[x].y });
            }
